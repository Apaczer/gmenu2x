...
void GMenu2X::main() {
	...
	platform->setCPU(confInt["cpuMenu"]);
#if defined(TARGET_TRIMUI)
	platform->setSDL();
	platform->setLCD();
#endif
...
bool GMenu2X::inputCommonActions(bool &inputAction) {
	...
	int HoldButton = SETTINGS;
	while (input[SETTINGS]) { // SETTINGS HOLD
		wasActive = HoldButton;

		input.update();

#if defined(TARGET_TRIMUI)	// SETTINGS + LR Disable (for keymon)
		if ((input[SECTION_PREV]) || (input[SECTION_NEXT])) {
		HoldButton = 0;
		}
#else
		if (SDL_GetTicks() - button_hold > 1000) {
...
#if defined(TARGET_TRIMUI)	// MENU + LR Disable (for keymon)
		if ((input[SECTION_PREV]) || (input[SECTION_NEXT])) {
		HoldButton = 0;
		}
#endif
		if (input[BACKLIGHT_HOTKEY]) {
			wasActive = BACKLIGHT; input[BACKLIGHT_HOTKEY] = false;
		} else if (input[VOLUME_HOTKEY]) {
			wasActive = VOLUP; input[VOLUME_HOTKEY] = false;
...
#if defined(TARGET_TRIMUI)
	int prevSDL = confInt["SDLPatch"];
	int prevlcd = confInt["lcdPatch"];
#endif
...
// https://www.man7.org/linux/man-pages/man3/setlocale.3.html
	...
	sd.addSetting(new MenuSettingMultiString(this, _("CPU settings"), _("Define CPU and overclock settings"), &tmp, opFactory, 0, MakeDelegate(this, &GMenu2X::cpuSettings)));
#if defined(TARGET_TRIMUI)
	sd.addSetting(new MenuSettingBool(this, _("SDL patch"), _("Allow SDL USBHP/tearing patch"), &confInt["SDLPatch"]));	//TRIMUI
	sd.addSetting(new MenuSettingBool(this, _("LCD patch"), _("Allow LCD R.Rate 69Hz patch"), &confInt["lcdPatch"]));	//
#endif
...
#if defined(TARGET_TRIMUI)
		if (prevSDL != confInt["SDLPatch"]) {
			platform->setSDL();
		}
		if (prevlcd != confInt["lcdPatch"]) {
			platform->setLCD();
		}
#endif
	}
...
void GMenu2X::cpuSettings() {
 	SettingsDialog sd(this, ts, _("CPU settings"), "skin:icons/configure.png");
	sd.addSetting(new MenuSettingBool(this, _("CPU clock change"), _("Allow CPU clock change"), &confInt["cpuChange"]));	//TRIMUI
 	sd.addSetting(new MenuSettingInt(this, _("Default CPU clock"), _("Set the default working CPU frequency"), &confInt["cpuMenu"], platform->cpu_menu, platform->cpu_min, platform->cpu_max, platform->cpu_step));
 	sd.addSetting(new MenuSettingInt(this, _("Maximum CPU clock"), _("Maximum overclock for launching links"), &confInt["cpuMax"], platform->cpu_max, platform->cpu_min, platform->cpu_max, platform->cpu_step));
 	sd.addSetting(new MenuSettingInt(this, _("Minimum CPU clock"), _("Minimum underclock used in Suspend mode"), &confInt["cpuMin"], platform->cpu_min, platform->cpu_min, platform->cpu_max, platform->cpu_step));
...
void GMenu2X::readConfig(string conffile, bool defaults) {
	if (defaults) {
		...
		confInt["lcdPatch"] = 0;	//TRIMUI
		confInt["SDLPatch"] = 0;	//TRIMUI
		confInt["cpuChange"] = 1;	//TRIMUI
	...
	evalIntConf(&confInt["lcdPatch"], 0, 0, 1 );		//TRIMUI
	evalIntConf(&confInt["SDLPatch"], 0, 0, 1 );		//TRIMUI
	evalIntConf(&confInt["cpuChange"], 1, 0, 1 );		//TRIMUI
	evalIntConf(&confInt["cpuMax"], platform->cpu_max, platform->cpu_min, platform->cpu_max );
	evalIntConf(&confInt["cpuMin"], platform->cpu_min, platform->cpu_min, platform->cpu_max );
	evalIntConf(&confInt["cpuMenu"], platform->cpu_menu, platform->cpu_min, platform->cpu_max );
	evalIntConf(&confInt["globalVolume"], 60, 0, 100 );
	evalIntConf(&confInt["backlight"], 70, 1, 100);
	evalIntConf(&confInt["minBattery"], 40, 1, 10000);
	evalIntConf(&confInt["maxBattery"], 49, 1, 10000);
...
void GMenu2X::writeConfig() {
	...
	for (ConfIntHash::iterator curr = confInt.begin(); curr != confInt.end(); curr++) {
		if (
			...
			(curr->first == "lcdPatch" && curr->second == 0) ||	//TRIMUI
			(curr->first == "SDLPatch" && curr->second == 0) ||	//TRIMUI
			(curr->first == "cpuChange" && curr->second == 1) ||	//TRIMUI
			(curr->first == "cpuMin" && curr->second == platform->cpu_min) ||
			(curr->first == "cpuMax" && curr->second == platform->cpu_max) ||
			(curr->first == "cpuMenu" && curr->second == platform->cpu_menu) ||
			(curr->first == "cpuLink" && curr->second == platform->cpu_link) ||
			(curr->first == "globalVolume" && curr->second == 60) ||
			(curr->first == "backlight" && curr->second == 70) ||
			(curr->first == "minBattery" && curr->second == 40) ||
			(curr->first == "maxBattery" && curr->second == 49) ||
...
void GMenu2X::skinMenu() {
	...
	do {
		if (prevSkin != confStr["skin"] || skinFontPrev != confStr["skinFont"]) {
			bool initwallpaperflag = true;
			if (prevSkin == "/") initwallpaperflag = false;
			...
			if (initwallpaperflag) confStr["wallpaper"] = wallpapers.at(0);
		}
...
void GMenu2X::poweroffDialog() {
	...
	mb.setButton(SECTION_PREV, _("TrimUI"));
	mb.setButton(SECTION_NEXT, _("Reboot"));
	mb.setButton(CONFIRM, _("PWoff"));
#else
...
#if defined(TARGET_TRIMUI)
		case SECTION_PREV: {
			MessageBox mb(this, _("TRIMUI Original MENU"));
			mb.setAutoHide(1);
			mb.exec();
			platform->hwDeinit2();
			setVolume(0);
			quit();
//			system("sync; killall updater; rm -rf /mnt/SDCARD/.tmp_update; reboot");
			system("rm -f /tmp/gmenu_exec; sync");
			exit(0);
			break;
		}
#endif
...
#if defined(TARGET_TRIMUI)
	bool MiyooButtonAssign = menu->getLinkApp()->getMiyooButtonAssign();
#endif
...
#if defined(TARGET_TRIMUI)
	sd.addSetting(new MenuSettingBool(this,			_("Miyoo Button Assign"),	_("Assign buttons to Miyoo standard"), &MiyooButtonAssign));
#endif
...
#if defined(TARGET_TRIMUI)
		menu->getLinkApp()->setMiyooButtonAssign(MiyooButtonAssign);
#endif
