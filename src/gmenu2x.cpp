...
#if defined(TARGET_MIYOO) || (TARGET_TRIMUI)
uint32_t oc_table[] = {
  0x00c81802,
  0x00cc1013,
  0x00cc1001,
...
#elif defined(TARGET_TRIMUI)
//int oc_choices[] = {200,408,528,640,720,736,744,756,768,792,800,816,828,832,864,896,900};
int oc_choices[] = {208,408,528,640,704,736,756,800,816,828,896,900};
#endif
...
#if !defined(TARGET_MIYOO) && !defined(TARGET_TRIMUI)
...
#if defined(TARGET_GP2X) || defined(TARGET_WIZ) || defined(TARGET_CAANOO) || defined(TARGET_RS97) || defined(TARGET_MIYOO) || defined(TARGET_TRIMUI)
	hwInit();
#endif
...
#elif defined(TARGET_MIYOO) || defined(TARGET_TRIMUI)
	SDL_ShowCursor(0);
	s->ScreenSurface = SDL_SetVideoMode(320, 240, confInt["videoBpp"], SDL_HWSURFACE | SDL_DOUBLEBUF);
	s->raw = SDL_CreateRGBSurface(SDL_SWSURFACE, resX, resY, confInt["videoBpp"], 0, 0, 0, 0);
#else
...
bool GMenu2X::inputCommonActions(bool &inputAction) {
	...
	while (input[MENU]) {
		wasActive = true;
		input.update();
#if defined(TARGET_TRIMUI)			//for TRIMUI
		if (input[SETTINGS]) {
			// BRIGHTNESS 		SELECT + START
			setBacklight(confInt["backlight"], true);
			return true;
		} else if (input[POWER]) {
			// VOLUME / MUTE	SELECT + MENU
...
#if defined(TARGET_TRIMUI)			//for TRIMUI
	wasActive = false;
	while (input[SETTINGS]) {		//wait until release start button
		wasActive = true;
		input.update();
	}
	input[SETTINGS] = wasActive;
#endif
...
#if defined(TARGET_TRIMUI)	//lcd init
		if ((memregs[0x0890 >> 2] & 1) != 0) { memregs[0x0890 >> 2] = memregs[0x0890 >> 2] & 0xF0FFFFFF | 0x03000000; }
#endif
	}
...
void GMenu2X::readConfig() {
	...
	evalIntConf( &confInt["cpuMax"], 900, 200, 1200 );
	evalIntConf( &confInt["cpuMin"], 208, 200, 1200 );
	evalIntConf( &confInt["cpuMenu"], 704, 200, 1200 );
	evalIntConf( &confInt["globalVolume"], 60, 1, 100 );
	evalIntConf( &confInt["videoBpp"], 16, 8, 32 );
	evalIntConf( &confInt["backlight"], 70, 1, 100);
	evalIntConf( &confInt["minBattery"], 40, 1, 50);
	evalIntConf( &confInt["maxBattery"], 49, 1, 50);
...
void GMenu2X::poweroffDialog() {
	...
	mb.setButton(SECTION_PREV, tr["TUI"]);
	mb.setButton(SECTION_NEXT, tr["Reboot"]);
	mb.setButton(CONFIRM, tr["PWoff"]);
	mb.setButton(CANCEL,  tr["Cancel"]);
#else
	mb.setButton(SECTION_NEXT, tr["Reboot"]);
	...
	if (response == CONFIRM) {
		...
		MessageBox mb(this, tr["TRIMUI Original MENU"]);
		mb.setAutoHide(500);
		mb.exec();
		setBacklight(0);
...
int GMenu2X::setVolume(int val, bool popup) {
	...
	if (popup) {
		...
		while (!close) {
			...
			else if ( input[SECTION_PREV] )	{ val -= volumeStep; if (val < 0) val = 100; }
			else if ( input[SECTION_NEXT] )	{ val += volumeStep; if (val > 100) val = 0; }
		}
		powerManager->resetSuspendTimer();
...
int GMenu2X::setBacklight(int val, bool popup) {
...
#if defined(TARGET_TRIMUI)
			if ( input[CONFIRM] || input[CANCEL] ) close = true;
			if ( input[LEFT] || input[SECTION_PREV] )	val = setBacklight(max(1, val - backlightStep), false);
			else if ( input[RIGHT] || input[SECTION_NEXT] )	val = setBacklight(min(100, val + backlightStep), false);
#else
  ...
  if(val == 0){
	...
	sprintf(buf, "echo %d > /sys/class/disp/disp/attr/lcdbl", lid);
	system(buf);
#endif
