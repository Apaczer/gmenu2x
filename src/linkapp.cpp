...
Link(gmenu2x, MakeDelegate(this, &LinkApp::run)), file(file) {
	...
	while (getline(f, line, '\n')) {
		...
		else if (name == "miyoobuttonassign" && value == "true") setMiyooButtonAssign(true);
#endif
...
#if defined(TARGET_TRIMUI)
	if (miyoobuttonassign)		f << "miyoobuttonassign=true"				<< std::endl;
#endif
...
#if defined(TARGET_TRIMUI)
	if (getMiyooButtonAssign() == true) gmenu2x->platform->set_gpiokeys("./gpio_keys_polled_miyoo.ko");
#endif
...
#if defined(TARGET_TRIMUI)
void LinkApp::setMiyooButtonAssign(bool value) {
	miyoobuttonassign = value;
	edited = true;
}
#endif
