...
uint32_t PowerManager::doSuspend(uint32_t interval, void *param) {
	if (interval > 0) {
		MessageBox mb(PowerManager::instance->gmenu2x, _("Suspend"));
		mb.setAutoHide(1);
		mb.exec();
		sync(); system("sync");
		...
		PowerManager::instance->gmenu2x->platform->setCPU(PowerManager::instance->gmenu2x->confInt["cpuMin"]);
		PowerManager::instance->suspendActive = true;
		return interval;
	}

	PowerManager::instance->gmenu2x->platform->setCPU(PowerManager::instance->gmenu2x->confInt["cpuMenu"]);
